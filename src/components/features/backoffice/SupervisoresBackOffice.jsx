...
const SupervisoresBackOffice = () => {
  ...
  const handleVerDetalles = async (supervisor) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/backoffice/supervisores/${supervisor.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleToggleSupervisorStatus = async (supervisorId, currentStatus, supervisorName) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem('token');
        await axios.patch(`${API_URL}/backoffice/supervisores/${supervisorId}/toggle-status`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
...
export default SupervisoresBackOffice;
